...
rm *.yaml && echo 'remove all yaml' & >/dev/null
### Pod Logs (5%)
[ -f /opt/KUTR00101/bar ] && rm /opt/KUTR00101/bar
echo 'Pod Logs clear'

bash -c "kubectl get pod > getpod.tmp"
bash -c "cat getpod.tmp |tail -n +2|cut -d ' ' -f 1 > rmpod.tmp"
...
kubectl replace -f pod-log.yaml --force
